...
class Uniform<T> {
    ...
    fetchUniformLocation(program: WebGLProgram, name) {
        assert(program);
        assert(name);
        if (!this.location) {
...
class Uniform1i extends Uniform<number> {
    ...
    set(program: WebGLProgram, name, v: number): void {
        this.fetchUniformLocation(program, name);
        if (this.current !== v) {
            this.current = v;
...
class Uniform2f extends Uniform<[number, number]> {
    ...
    set(program: WebGLProgram, name, v: [number, number]): void {
        this.fetchUniformLocation(program, name);
...
class Uniform3f extends Uniform<[number, number, number]> {
    ...
    set(program: WebGLProgram, name, v: [number, number, number]): void {
        this.fetchUniformLocation(program, name);
...
class Uniform4f extends Uniform<[number, number, number, number]> {
    ...
    set(program: WebGLProgram, name, v: [number, number, number, number]): void {
        this.fetchUniformLocation(program, name);
        if (v[0] !== this.current[0] || v[1] !== this.current[1] ||
...
class UniformColor extends Uniform<Color> {
    ...
    set(program: WebGLProgram, name, v: Color): void {
        this.fetchUniformLocation(program, name);
...
class UniformMatrix4f extends Uniform<Float32Array> {
    ...
    set(program: WebGLProgram, name, v: Float32Array): void {
        this.fetchUniformLocation(program, name);
